...
def array_equaliser(output1, output2, output3):
    problemdata = [output1, output2, output3]
    problemdata_len = [len(output1), len(output2), len(output3)]
    sorted_problemdata = []
    while len(problemdata) > 0:
        x = 0
        for i in range(len(problemdata_len)):
            i = i - x
            if problemdata_len[i] == max(problemdata_len):
                problemdata_len.pop(i)
                sorted_problemdata.append(problemdata.pop(i))
                x += 1
    for i in range(1, len(sorted_problemdata)):
        for j in np.arange(len(sorted_problemdata[0]) - len(sorted_problemdata[i])).tolist():
            sorted_problemdata[i].append("")
...
FYTD = np.where(dataset == 'Monthly % - YTD')[1]
...
problemdataFYTD = ["The following is problematic FYTD data:", ""]
...
output_list3 = [""], [""], [""], [""]
output_array3 = np.array(output_list3)

FYTD_dates = [i for i, s in enumerate(dates) if '06-30' in s]
...
for i in OptionList:
    try:
        index_creator(FYTD_dates[0], i, FYTD, problemdataFYTD)
    except (TypeError, KeyError):
        if i not in missingdataFYTD:
            missingdataFYTD.append(i)

print(missingdataFYTD)

problemdata3m[2:] = sorted(problemdata3m[2:])
problemdata12m[2:] = sorted(problemdata12m[2:])
problemdataFYTD[2:] = sorted(problemdataFYTD[2:])

problemdatefinder(3, problemdata3m)
problemdatefinder(12, problemdata12m)
problemdatefinder(FYTD_dates[0], problemdataFYTD)

array_equaliser(problemdata3m, problemdata12m, problemdataFYTD)
...
output_array3 = Output_Summary_Creator(output_array3, problemdataFYTD)
output_array_final = np.column_stack((output_array, output_array2, output_array3))
